...
export default function PostDetail() {
  ...
  useEffect(() => {
    ...
    if (route.query.id !== undefined) {
      const URL = Backend.BASE_URL + "/posts/" + route.query.id;
  ...
  const DetailPost = () => {
    return (
      <Box className="detail-posts-focus-content">
        <div
          className="detail-posts-focus"
          dangerouslySetInnerHTML={{
            __html: detailPost?.content ? detailPost.content : "",
          }}
        ></div>
      </Box>
    );
  ...
  const Directory = () => {
    const PostTypeDirectory = () => {
      const link =
        "/posts?type=" +
        findTypePostName(detailPost?.typeId) +
        "&page=0&pageSize=9";
      return (
  ...
  const CommentComponent = () => {
    ...
    const getComment = (postId: any) => {
      axios({
        method: "get",
        url: Backend.BASE_URL + "/comment/" + postId,
      }).then(
        (res) => {
          console.log(res.data);
    ...
    const PostComment = () => {
      ...
      return (
        ...
        >
          ...
          />
          <Button className="email-button" onClick={() => createComment()}>
            Bình luận
          </Button>
        </Box>
      );
    ...
    return (
      <Box>
        <p
          style={{ color: "rgb(0,32,96)", fontSize: "20px", fontWeight: "700" }}
        >
          Bình luận
        </p>
        <Box
    ...
    <Page title={PAGE_TITLE.HOME} menuIndex={0}>
      <Box className="post-detail-content-wrapper">
        <Box className="post-detail-title-wrapper">
          <Directory></Directory>
          <Box className="posts-detail-title">
            <p
              style={{
                fontSize: "30px",
