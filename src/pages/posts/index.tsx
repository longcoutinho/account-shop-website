...
export default function PostComponent() {
  ...
  const redirectPagination = (page: string, pageSize: any, typePost: any) => {
    ...
    if (typePost != "" && typePost != undefined)
      params.append("type", typePost);
    push({
      pathname: "/posts",
      search: "?" + params,
    });
    ...
    return (
      <Box className="related-post-container">
        {listPosts.slice(0, 5).map((relatedPost, index) => (
          <Box
            key={index}
            onClick={() => redirect(relatedPost.id)}
            className="related-post-element"
          >
  ...
  const ListFeaturedPosts = () => {
    ...
    const ListFeaturedPostsComponent = listFeaturedPosts
      .slice(0, 4)
      .map((featuredPost, index) => (
        <Box key={index}>
          <SwiperSlide
            onClick={() => redirect(featuredPost.id)}
            key={index}
            className="swiper-slide-featured-posts"
          >
            <div className="slide-post">
              ...
              <div className="swiper-slide-featured-posts-content">
                ...
                <p className="post-page-para-intro">
    ...
    const ListReadMost = listFeaturedPosts.slice(0, 4).map((post, index) => (
      <Box key={index} className="list-read-most-content">
        ...
        <p
          ...
          className="list-read-most-title"
                ...
                <Box className="image-and-content-element">
                  ...
                  <Box className="list-posts-content">
                    <h1
                      ...
                      className="title-list-posts-element"
                    >
                      {post.title}
                    </h1>
                    <p
      ...
      >
        <ListPostComponents />
        {
          <PaginationMui
            ...
            variant="outlined"
            onChange={(e: any, value) => paginationChange(value)}
            sx={{ color: "white" }}
          />
        }
      </Box>
  ...
  const Directory = () => {
        ...
        return (
          <Box sx={{ display: "flex", flexDirection: "row" }}>
            ...
            <a style={{ marginLeft: "5px" }} href="#">
              Tìm kiếm với từ khóa `&apos;`{query.title}`&apos;`
            </a>
          </Box>
        );
      }
    ...
    return (
      <Box className="directory-wrapper">
        <a href="./">Trang chủ</a>
        <p className="directiory-icon"> {" >> "} </p>
        <Link style={{ marginLeft: "5px" }} href="/posts?page=0&pageSize=9">
          Kho tàng tri thức
        </Link>
          ...
          <Box className="menu-type-post-content">
            {listTypePost.map((typePost, index) => (
              <Box
                ...
                className="menu-type-post-element"
                key={index}
              >
                <p>{typePost.name}</p>
              </Box>
